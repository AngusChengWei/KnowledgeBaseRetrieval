    ...
    <el-card shadow="never">
      <template #header>
        <div class="card-header">
          <div>
            <h3>部门知识库</h3>
            <p class="sub-text">管理各部门专属知识库文档</p>
          </div>
          <div class="header-actions">
            <el-select v-model="selectedDept" style="width: 160px" @change="loadDocs">
              <el-option
      ...
      <el-empty v-if="documents.length === 0" description="当前部门暂无文档" />
...
import { getDepartments, uploadDocuments, listDocuments, deleteDocument, rebuildKnowledgeBase, rebuildFileVectors, deleteFileVectors, importUrl, getDocumentContent, updateDocumentTitle } from '../../api/chat'
...
watch(() => authStore.currentOrgId, async () => {
  ...
  departments.value = data.departments.filter(d => d.name !== 'general')
...
async function handleUpload(file) {
  ...
  try {
    const data = await uploadDocuments([file.raw], selectedDept.value)
...
async function handleRebuild() {
  ...
  try {
    const data = await rebuildKnowledgeBase(selectedDept.value)
...
async function handleDelete(filename) {
  try {
    await deleteDocument(filename, selectedDept.value)
...
async function handleImportUrl() {
  ...
  try {
    const data = await importUrl(importUrlInput.value.trim(), selectedDept.value)
...
async function handleRebuildFile(row) {
  ...
  try {
    ...
    if (row.source_type === 'url') {
      // URL 条目：重新抓取并导入
      data = await importUrl(row.source_ref || filename, selectedDept.value)
    } else {
      data = await rebuildFileVectors(filename, selectedDept.value)
    }
...
async function handleDeleteVectors(filename) {
  ...
  try {
    await deleteFileVectors(filename, selectedDept.value)
...
async function handleViewContent(row) {
  ...
  try {
    const data = await getDocumentContent(row.filename, selectedDept.value)
...
async function handleSaveTitle() {
  ...
  try {
    await updateDocumentTitle(titleEditFilename.value, titleEditInput.value.trim(), selectedDept.value)
